...
StackedLayout::StackedLayout( flags layoutHints )
{
...
void
StackedLayout::doLayout( Container& aContainer, const GraphicsContext& gc ) const
{
	...
	if ( LayoutManager::EXPANDED == (LayoutManager::EXPANDED & this->hints ))
	{
		...
		while( it->hasNext() )
		{
			...
			dim = com.getPreferredDimensions( gc, container_width, container_height );
	...
	case LayoutManager::NORTHWARD:
		{
			...
			for ( unsigned int i=nr_of_elements; i > 0; i-- )
			{
				...
				case 0:
					{
						dim = com.getPreferredDimensions( gc, container_width, offset );
	...
	default:
		{
			...
			while ( it->hasNext() )
			{
				...
				unsigned int width  = container_width;
				unsigned int height;
				...
				com.setBounds( x, y + offset, width, height );
				com.doLayout( gc );
				offset += height;
			}
			delete it;
 ...
 */
 /*
unsigned int
StackedLayout::preferredLayoutWidth( const Sequence& components,
...
unsigned int
StackedLayout::preferredLayoutHeight( const Sequence& components,
                                      const GraphicsContext& gc ) const
{
...
*/
Dimensions
StackedLayout::preferredLayoutDimensions( const GraphicsContext& gc, const Container& aContainer, unsigned int width, unsigned int height ) const
{
	Dimensions d;
	...
	while ( it->hasNext() )
	{
		...
		d.width = Math::max( d.width, component.width );
