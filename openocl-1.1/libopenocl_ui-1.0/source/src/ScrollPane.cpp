...
ScrollPane::ScrollPane( Component& com, unsigned int width, unsigned int height, bool buffered )
: Pane( width, height )
{
	this->setEdge( 5 );
...
void
ScrollPane::deliverEvent( const Event& anEvent )
{
	flags type = anEvent.getType();
	if ( Event::CHANGE_EVENT == type )
	{
		//XXX this->setRedrawAll( true );
...
void
ScrollPane::processEvent( const Event& anEvent )
{
	flags type = anEvent.getType();
...
void
ScrollPane::setScrollerBackground( const Color& aColor )
{
	this->scroller->setBackground( aColor );
}

void
ScrollPane::setComponent( Component& aComponent )
{
...
void
ScrollPane::setConstrainWidth( bool constrain )
{
	this->scroller->setConstrainWidth( constrain );
}

void
ScrollPane::setConstrainHeight( bool constrain )
{
	this->scroller->setConstrainHeight( constrain );
}

void
ScrollPane::resetOffsets()
{
...
void
ScrollPane::setOffsets( unsigned int x, unsigned int y )
{
...
void
ScrollPane::doLayout( Container& aContainer, const GraphicsContext& gc ) const
{
...
void
ScrollPane::doLayout( const GraphicsContext& gc )
{
	/*
...
Dimensions
ScrollPane::preferredLayoutDimensions( const GraphicsContext& gc, const Container& aContainer, unsigned int width, unsigned int height ) const
{
	IO::err().println( "ScrollPane::preferredLayoutWidth() should not be called\n" );
	abort();
	Dimensions dim;
	dim.width = 0xFFFF;
	dim.height = 0xFFFF;
...
//Dimensions
//ScrollPane::getPreferredDimensions( const GraphicsContext& gc, unsigned int width, unsigned int height ) const
//{
//	return Dimensions( width, height );
//}

bool
ScrollPane::draw( GraphicsContext& gc ) const
{
	...
	//fprintf( stderr, "ScrollPane::draw()\n" );
	...
	if ( changed )
	{
		...
		//const_cast<ScrollPane*>( this )->setChanged( false );
